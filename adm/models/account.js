...
var accountModel =
{
    login:
    {
        authenticate: function( request, response, callback, klay )
        {
            ...
            // The query for admin users is slightly different. We also join the elevated table and fetch the stored
            // password from there. This is because if the user was created as an admin then the users password will
            // be associated with their id within the elevated table. If they are properly given access to the app
            // as an admin, the elevated table will be populated respectively. It's simply another security check for
            // the admin site.
            var authenticated = db
            .query( "select users.id as id, users.type as type, users.ts as ts, username, elevated.secure as password, context, contexts.acl as acl, company, first, " +
                    "last, contexts.email as email from users join elevated on users.id=elevated.user join contexts on users.id=contexts.owner and" + " users.context = contexts.id " +
                    "where username='" + user + "';" )
            .execute();
